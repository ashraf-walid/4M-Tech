...
export default function CheckoutModal({ onClose, product }) {
  ...
  const handleApplyCoupon = async (code) => {
    ...
    if (!coupon) {
      setErrorMsg('كود الخصم غير صحيح');
    ...
    // تحقق من التفعيل
    if (!coupon.isActive) {
      setErrorMsg('هذا الكوبون غير مفعل.');
      setTimeout(() => setErrorMsg(''), 2500);
      throw new Error('الكوبون غير مفعل');
    }
    ...
    if (coupon.expiryDate) {
      ...
      if (now > expiry) {
        setErrorMsg('انتهت صلاحية هذا الكوبون.');
        setTimeout(() => setErrorMsg(''), 2500);
    ...
    setInfoMsg('تم تطبيق الكوبون بنجاح');
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      setErrorMsg('فشل تسجيل الدخول. حاول مرة أخرى.');
      setTimeout(() => setErrorMsg(''), 2500);
      console.error(error);
    } finally {
  ...
  const handleSubmitOrder = async () => {
    try {
      ...
      const validDiscount = discount ? Math.abs(discount) : 0;
      ...
      setErrorMsg('فشل معالجة الطلب. حاول مرة أخرى.');
      setTimeout(() => setErrorMsg(''), 2500);
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl flex flex-col max-h-[95vh]">
        ...
        {/* رسائل الخطأ أو المعلومات */}
        {(errorMsg || infoMsg) && (
          <div className={`my-3 px-4 py-2 rounded text-center font-semibold ${errorMsg ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-blue-100 text-blue-700 border border-blue-300'}`}>
            {errorMsg || infoMsg}
          </div>
        )}
