...
export default function CouponSection({ onApplyCoupon }) {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message: `تم تطبيق الكوبون! خصم ${result.discount}%`,
  ...
  return (
    <div className="space-y-3 border-t pt-4 mt-4 text-right">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-gray-700">هل لديك كوبون؟</h3>
        {status?.type === 'success' && (
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pr-10 pl-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-right"
          />
        </div>
        <button
        ...
        >
          {isChecking ? <Loader2 className="w-5 h-5 animate-spin" /> : 'تطبيق'}
        </button>
      </div>
